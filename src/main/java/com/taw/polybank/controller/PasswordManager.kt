 ...
 */
class PasswordManager(private val clientService: ClientService) {
  ...
  fun savePassword(client: ClientDTO, plainPassword: String?): Array<String?> {
  ...
  fun verifyPassword(client: ClientDTO?, password: String?): Boolean {
    var result = false
    if (client != null) {
  ...
  fun resetPassword(client: ClientDTO, newPassword: String?) {
    val salt = clientService.getSalt(client.getId())
    val seed: ByteArray = salt.toByteArray(StandardCharsets.ISO_8859_1)
    primeRandom(seed)
    initializeEncoder()
