...
class ATMController {
  ...
  @GetMapping("/")
  fun doIndex(model: Model, session: HttpSession): String {
  ...
  fun gotoEditarDatos(model: Model, session: HttpSession): String {
    val client = session.getAttribute("client") as ClientDTO?
    if (client == null) {
      return "atm/index"
    } else {
  ...
  fun doEditarDatos(
    @ModelAttribute("client") client: ClientDTO, model: Model, session: HttpSession
  ): String {
  ...
  @GetMapping("/takeOut")
  fun menuTakeOut(model: Model, session: HttpSession): String {
    if (session.getAttribute("client") == null || session.getAttribute("bankAccount") == null) return "atm/index"
  ...
  fun doTakeOut(
    ...
    transactionService!!.makeTransaction(
      amount, bankAccount.iban, client.getName(), badge, emisorBadge, bankAccount, client
    )
  ...
  fun listTransactions(session: HttpSession, model: Model): String {
    if (session.getAttribute("client") == null || session.getAttribute("bankAccount") == null) return "atm/index"
  ...
  fun filterTransactions(
    session: HttpSession, model: Model, @ModelAttribute("filter") filter: TransactionFilterLucia
  ): String {
    if (session.getAttribute("client") == null || session.getAttribute("bankAccount") == null) return "atm/index"
  ...
  fun doMakeUnbanPetition(
    session: HttpSession, @RequestParam("description") description: String?
  ): String {
    val client = session.getAttribute("client") as ClientDTO?
    val bankAccount = session.getAttribute("bankAccount") as BankAccountDTO?
    if (client == null || bankAccount == null) {
