...
public class ATMController {
  ...
  @Autowired private SuspiciousAccountService suspiciousAccountService;
  ...
  public String doEditarDatos(
      @ModelAttribute("client") ClientDTO client, Model model, HttpSession session) {
  ...
  public String doListarAcciones(
      @RequestParam(name = "bankAccount", required = false) Integer bankAccountId,
      HttpSession session) {
  ...
  public String doMakeTransfer(
      @RequestParam("amount") double amount,
      @RequestParam("receiver") String receiverIBAN,
    ...
    if (suspiciousAccountService.isSuspicious(receiverIBAN)) {
      model.addAttribute(
          "error", "The destination account is suspicious. You cannot transfer money to it.");
      return "atm/bankAccount_transferMenu";
    }
    if (bankAccountReceiver != null
        && !bankAccountReceiver.getClientByClientId().getName().equals(receiverName)) {
      model.addAttribute(
          "error", "The name of the proprietary of the destination account is not correct.");
      return "atm/bankAccount_transferMenu";
    }
    ...
    transactionService.makeTransaction(
        amount, receiverIBAN, receiverName, badgeReceiver, emisorBadge, bankAccountEmisor, client);
  ...
  @PostMapping("/takeOut")
  public String doTakeOut(
      @RequestParam("amount") double amount,
      @RequestParam("badge") int badgeId,
      HttpSession session) {
    if (session.getAttribute("client") == null || session.getAttribute("bankAccount") == null)
      return "atm/index";
    ...
    transactionService.makeTransaction(
        amount, bankAccount.getIban(), client.getName(), badge, emisorBadge, bankAccount, client);
  ...
  public String listTransactions(HttpSession session, Model model) {
    ...
    TransactionFilterLucia filter =
        new TransactionFilterLucia(
            Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now()), "", "", "0.0");
  ...
  public String filterTransactions(
      HttpSession session, Model model, @ModelAttribute("filter") TransactionFilterLucia filter) {
    if (session.getAttribute("client") == null || session.getAttribute("bankAccount") == null)
      return "atm/index";
  ...
  public String doMakeUnbanPetition(
      HttpSession session, @RequestParam("description") String description) {
    ClientDTO client = (ClientDTO) session.getAttribute("client");
    BankAccountDTO bankAccount = (BankAccountDTO) session.getAttribute("bankAccount");
    if (client == null || bankAccount == null) {
      return "atm/index";
