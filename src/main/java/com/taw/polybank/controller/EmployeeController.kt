...
class EmployeeController {
  ...
  fun postLogin(
    ...
    val employee_opt = employeeRepository!!.findByDNI(dni)
    if (employee_opt.isPresent()) {
      val employee = employee_opt.get()
      session.setAttribute("employee", employee)
      if (employee.getType().toString() == "assistant") return ("redirect:/employee/assistance")
      else if (employee.getType().toString() == "manager") return ("redirect:/employee/manager")
      session.setAttribute("employee", employee)
    }
  ...
  fun getActions(model: Model?, session: HttpSession): String {
    if (session.getAttribute("employee") == null) return "redirect:/employee"
    return ("employee/manager/actions")
  }

  @GetMapping("manager/requests")
  fun getRequests(session: HttpSession, model: Model): String {
    if (session.getAttribute("employee") == null) return ("redirect:/employee")
  ...
  fun getCompanyAccount(@PathVariable("id") id: Int?, model: Model): String {
