...
public class UserCompany {
  @Autowired protected AuthorizedAccountService authorizedAccountService;
  @Autowired protected BadgeService badgeService;
  @Autowired protected BankAccountService bankAccountService;
  @Autowired protected BeneficiaryService beneficiaryService;
  @Autowired protected ClientService clientService;
  @Autowired protected CompanyService companyService;
  @Autowired protected CurrencyExchangeService currencyExchangeService;
  ...
  public String blockedUserMenu(Model model, HttpSession session) {
    ...
    List<RequestDTO> requests =
        requestService.findUnsolvedUnblockRequestByUserId(client.getId(), bankAccount.getId());
  ...
  public String allegation(@RequestParam("msg") String message, HttpSession session, Model model) {
    ...
    model.addAttribute(
        "message", "Allegation successfully submitted. Wait patiently for its resolution.");
  ...
  public String setUpPassword(@ModelAttribute("client") ClientDTO client, Model model) {
    model.addAttribute("client", client);
  ...
  private void updateUser(ClientDTO client, String password, HttpSession session, Model model) {
    ...
    authorizedAccountService.save(
        authorizedAccount, clientService, bankAccountService, badgeService);

    bankAccountService.addAuthorizedAccount(bankAccount, authorizedAccount);
  }

  @GetMapping("/editMyData")
  public String editMyData(HttpSession session, Model model) {
    ClientDTO client = (ClientDTO) session.getAttribute("client");
    model.addAttribute("client", client);
    return "/company/newRepresentative";
  }
  ...
  public String updateCompanyData(
      @ModelAttribute("company") CompanyDTO company, HttpSession session, Model model) {
  ...
  public String listFilteredRepresentatives(
      @ModelAttribute("clientFilter") ClientFilter clientFilter, Model model, HttpSession session) {
      ...
      if (clientFilter.getNameOrSurname().isBlank()) {
        clientList =
            clientService.findAllRepresentativesOfACompanyThatWasRegisteredBetweenDates(
                company.getId(), registeredBefore, registeredAfter);
      } else {
        clientList =
            clientService
  ...
  public String blockRepresentative(@RequestParam("id") Integer userId, HttpSession session) {
    CompanyDTO company = (CompanyDTO) session.getAttribute("company");
  ...
  public String processTransfer(
    ...
    if (amount <= 0) {
      ...
      model.addAttribute(
          "message", "Money transfer was denied, invalid amount (amount = " + amount + ")");
      return "/company/userHomepage";
    }
    ...
    if (bankAccount.getBalance() < amount) {
      ...
      model.addAttribute(
          "message", "Money transfer was unsuccessful, not enough money in your bank account");
      return "/company/userHomepage";
    }
    ...
    if (recipientBankAccount != null) { // Internal bank money transfer
      ...
      if (companyRecipient
          == null) { // Private Client is going to receive money, Authorized person can not figure
        ...
        if (!clientRecipient.getName().equals(beneficiaryName)) {
          ...
          model.addAttribute(
              "message", "Money transfer was unsuccessful, recipient name is not correct");
          return "/company/userHomepage";
        }
      ...
      if (originBadge.getId() != recipientBadge.getId()) { // Do we need currency exchange?
        ...
        recipientBankAccount.setBalance(
            recipientBankAccount.getBalance() + currencyExchange.getFinalAmount());
      } else {
      ...
      if (beneficiary == null) {
        recipientBadge =
            badgeService
                .getRandomBadge(); // Assign random badge due to it unknown, and we can simulate
    ...
    model.addAttribute(
        ...
        amount
            + " "
            + bankAccount.getBadgeByBadgeId().getName()
  ...
  public String makeExchange(
      @ModelAttribute BadgeDTO targetBadge, HttpSession session, Model model) {
    CompanyDTO company = (CompanyDTO) session.getAttribute("company");
    ClientDTO client = (ClientDTO) session.getAttribute("client");
    BankAccountDTO bankAccount = company.getBankAccountByBankAccountId();
    ...
    BenficiaryDTO beneficiary =
        defineBeneficiary(company.getName(), bankAccount.getIban(), targetBadge);
    ...
    if (currentBadge.getId() != targetBadge.getId()) {
      CurrencyExchangeDTO currencyExchange =
          defineCurrencyExchange(
              currentBadge, targetBadge, bankAccount.getBalance(), transaction, payment);
      transaction.setPaymentByPaymentId(payment);
  ...
  public String operationHistory(
      @ModelAttribute("transactionFilter") TransactionFilterIllya transactionFilter,
      HttpSession session,
      Model model) {
    return operationHistoryFilters(transactionFilter, session, model);
  }

  private String operationHistoryFilters(
      TransactionFilterIllya transactionFilter, HttpSession session, Model model) {
    ...
    if (transactionFilter == null) {
      transactionList =
          transactionService.findTransactionsByBankAccountByBankAccountIdId(bankAccount.getId());
  ...
  private void updateBadges(
      BadgeDTO originBadge, BadgeDTO recipientBadge, CurrencyExchangeDTO currencyExchange) {
    badgeService.addAndSave(originBadge, currencyExchange);
    badgeService.addAndSave(recipientBadge, currencyExchange);
  }

  private CurrencyExchangeDTO defineCurrencyExchange(
      BadgeDTO originBadge,
      BadgeDTO recipientBadge,
      Double amount,
  ...
  private BenficiaryDTO defineBeneficiary(
      String beneficiaryName, String iban, BadgeDTO recipientBadge) {
