...
class ClientAssistanceController {
  ...
  @PostMapping("/newChat")
  fun doNewChat(model: Model, session: HttpSession): String {
    val client = this.clientService!!.findById((session.getAttribute("client") as ClientDTO).getId())
    if (client.isPresent()) {
      val chat = ChatDTO()
  ...
  @PostMapping("/send")
  fun doSend(@RequestParam("content") content: String?, @RequestParam("chatId") chatId: Int?): String {
  ...
  @PostMapping("/close")
  fun doSend(@RequestParam("chatId") chatId: Int?): String {
    val chat = chatService!!.findById(chatId)
