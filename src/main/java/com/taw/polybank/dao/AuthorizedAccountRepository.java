 ...
 */
@Repository
public interface AuthorizedAccountRepository
    extends JpaRepository<AuthorizedAccountEntity, Integer> {

  @Query("select c.authorizedAccountsById from BankAccountEntity c where c.id = :bankAccountId")
  List<AuthorizedAccountEntity> findAuthorizedAccountEntitiesOfGivenBankAccount(
      @Param("bankAccountId") Integer id);
  ...
  AuthorizedAccountEntity findAuthAccOfGivenClientAndCompany(
      @Param("clientId") Integer clientId, @Param("companyId") Integer companyId);
}
