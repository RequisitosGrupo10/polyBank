...
public interface ClientRepository extends JpaRepository<ClientEntity, Integer> {
  ...
  @Query(
      "select c from ClientEntity c where c.name like concat('%',:name ,'%') or c.surname like concat('%',:name ,'%')")
  ...
  @Query(
      "select c from ClientEntity c where c.dni = :dni and c.name like concat('%',:name ,'%') or c.surname like concat('%',:name ,'%')")
  List<ClientEntity> findClientsByDNIAndName(@Param("dni") String dni, String name);

  @Query(
      "select ce from ClientEntity ce join ce.authorizedAccountsById aa join aa.bankAccountByBankAccountId bank "
  ...
  @Query(
      "SELECT ce FROM ClientEntity ce "
          ...
          + "AND ce.creationDate <= :registeredBefore "
          + "AND ce.creationDate >= :registeredAfter")
  List<ClientEntity> findAllRepresentativesOfACompanyThatWasRegisteredBetweenDates(
      @Param("companyId") Integer id,
      @Param("registeredBefore") Timestamp registeredBefore,
      @Param("registeredAfter") Timestamp registeredAfter);
  ...
  @Query(
      "select count(auth) from ClientEntity c join c.authorizedAccountsById auth where c.id = :clientID")
