...
public class BankAccountService {
  ...
  @Autowired private AuthorizedAccountRepository authorizedAccountRepository;
  ...
  private List<BankAccountDTO> bankAccountEntityListToDTO(
      List<BankAccountEntity> bankAccountEntityList) {
  ...
  public BankAccountEntity toEntity(
      BankAccountDTO bankAccount, ClientService clientService, BadgeService badgeService) {
    BankAccountEntity bankAccountEntity =
        bankAccountRepository.findById(bankAccount.getId()).orElse(new BankAccountEntity());
    ...
    bankAccountEntity.setClientByClientId(
        clientService.toEntidy(bankAccount.getClientByClientId()));
  ...
  public int getBankAccountId(BankAccountDTO bankAccount) {
    BankAccountEntity bankAccountEntity =
        bankAccountRepository.findBankAccountEntityByIban(bankAccount.getIban());
    return bankAccountEntity.getId();
  }

  public void addAuthorizedAccount(
      BankAccountDTO bankAccount, AuthorizedAccountDTO authorizedAccount) {
    BankAccountEntity bankAccountEntity =
        bankAccountRepository.findById(bankAccount.getId()).orElse(null);
    AuthorizedAccountEntity authorizedAccountEntity =
  ...
  public void save(
      BankAccountDTO bankAccountDTO, ClientService clientService, BadgeService badgeService) {
